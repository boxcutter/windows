...
call :run reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\WindowsUpdate\Auto Update" /v EnableFeaturedSoftware /t REG_DWORD /d 0 /f

call :run reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\WindowsUpdate\Auto Update" /v IncludeRecommendedUpdates /t REG_DWORD /d 0 /f

:: source: https://github.com/StefanScherer/packer-windows/blob/my/scripts/dis-updates.bat

rem http://www.windows-commandline.com/disable-automatic-updates-command-line/
call :run reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\WindowsUpdate\Auto Update" /v AUOptions /t REG_DWORD /d 0 /f
...
if %ERRORLEVEL% equ 0 (
    call :run reg delete "HKLM\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate" /f
)

rem even harder, disable windows update service
call :run sc config wuauserv start= disabled

sc query wuauserv | findstr /R "STATE.*RUNNING" >nul 2>nul

if %ERRORLEVEL% equ 0 (
    call :run net stop wuauserv
)
