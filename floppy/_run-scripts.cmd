...
:: @TODO remove this
move /y "%TEMP%\set@run-scripts.txt" "%PACKER_TMP%\"
set >"%PACKER_TMP%\set@run-scripts2.txt"
:: @TODO remove this
...
if not defined TEE_CMD (
    if exist "a:\01-install-tee.cmd" (
        call "a:\01-install-tee.cmd"
        call :set_tee_cmd
    )
)

if defined PACKER_RUN (
    if not exist "%PACKER_RUN%" (
        set PACKER_RUN=
    )
)
...
if not defined PACKER_RUN (
    for /l %%i in (1,1,99) do (
        if not defined PACKER_RUN (
            if not exist "%PACKER_TMP%\%~n0-scripts-%%i.txt" (
                set PACKER_RUN=%PACKER_TMP%\%~n0-scripts-%%i.txt
            )
        )
    )
)
...
set _scripts_to_run=%*
if not defined _scripts_to_run (
    if defined PACKER_SCRIPTS_TO_RUN (
        set _scripts_to_run=%PACKER_SCRIPTS_TO_RUN%
    )
)

dir /b /on %_scripts_to_run% | findstr /v "^_" | findstr /i /v "00-run-all-scripts.cmd" >"%PACKER_RUN%"

set _scripts_to_run=

popd

set _packer_run_tmp=%PACKER_RUN%.tmp

for %%i in (%PACKER_SKIP%) do (
    findstr /v /r "^[space]*%%~i[space]*$" "%PACKER_RUN%" >"%_packer_run_tmp%"
    move /y "%_packer_run_tmp%" "%PACKER_RUN%"
)

set _packer_run_tmp=

set _scripts=0
for /f %%i in (%PACKER_RUN%) do (
...
type "%PACKER_RUN%"
echo ---------->>"%PACKER_LOG%"
type "%PACKER_RUN%" >>"%PACKER_LOG%"
echo ---------->>"%PACKER_LOG%"

set _script_no=0
for /f %%i in (%PACKER_RUN%) do (
    set /a "_script_no+=1" >nul
    title %0 %*: executing script !_script_no! of %_scripts%: %%i
    call :info %~nx0: Executing script !_script_no! of %_scripts%: %%i
    call :run_script "%%~i" !_script_no!
)

set _script_no=

title %0 %*: finished executing %_scripts% scripts
call :info %~nx0: exiting with 0
goto exit0
...
:run_script
    ...
    if /i "%~x1" == ".ps1" (
        for %%j in (v3.0 v1.0) do (
            if not defined _cmd (
                if exist %SystemRoot%\System32\WindowsPowerShell\%%j\powershell.exe (
                    set _cmd=%SystemRoot%\System32\WindowsPowerShell\%%j\powershell.exe -File "%~dp0\%~nx1"
                )
            )
        )
    )
    ...
    ("%ComSpec%" %_cmd_opts% /e:on /v:on /c "%_cmd%" 2>&1 || echo !ERRORLEVEL! >"%PACKER_TMP%\%~n1.err") | %TEE_CMD% "%PACKER_TMP%\%~n1.log"

    popd

    if exist "%PACKER_TMP%\%~n1.err" (
        call :error %~nx0: %1 returned an error [see "%PACKER_TMP%\%~n1.log" for details]
        move /y "%PACKER_TMP%\%~n1.err" "%PACKER_TMP%\%~n1-%2.err"
    ) else (
        call :info %~nx0: %1 completed successfully.
    )
    goto :eof
