...
if "%~1" == "" (
    call :error %0 called without a parameter
    goto exit1
)

set _taskfile=%PACKER_TMP%\schtask_no.txt
if exist "%_taskfile%" (
    set /p _taskno=<"%_taskfile%" >nul
)
set /a "_taskno+=1" >nul
echo>"%_taskfile%" %_taskno%
set _taskfile=

set _tr=%*
set _tn=packer_task_%_taskno%
set _taskno=

schtasks /Query /TN "%_tn%" >nul 2>nul

if %ERRORLEVEL% equ 0 (
    call :run schtasks /Delete /TN "%_tn%" /F
)

call :run schtasks /Create /TN "%_tn%" /TR "%_tr%" /RU "%USERNAME%" /RP "%PACKER_PASSWORD%" /F /IT /RL HIGHEST /SC ONCE /SD 01/01/2038 /ST 00:00 /V1

call :sleep 2

call :run schtasks /Run /TN "%_tn%"
...
call :wait_while Ready

call :wait_while Running
...
:wait_while
    set _waitfile=%USERPROFILE%\Desktop\DeleteToContinue.txt
    echo Remove this file to continue >"%_waitfile%"
    set _seconds_left=%PACKER_TIMEOUT%
    echo Waiting %_seconds_left% seconds for task %_tn% to leave '%1' state, or for %_tempfile% to disappear...

    :_wait_loop
        shutdown /a >nul 2>nul
        ver >nul
        if not exist "%_waitfile%" (
            goto _wait_killed
        )
        if %_seconds_left% leq 0 (
            goto _wait_timed_out
        )
        ver >nul
        schtasks /query /NH /TN "%_tn%" 2>nul | findstr /B /I "%_tn%" | findstr "\<%~1\>" >nul 2>nul
        if %ERRORLEVEL% neq 0 (
            goto _wait_done
        )
        call :sleep 1
        set /a "_seconds_left-=1" >nul
        if "%PACKER_DEBUG%" geq "5" (
            @set /p ".=Waiting %_seconds_left% more seconds for task %_tn% to leave '%1' state...!_cr!" <nul
        )
        goto _wait_loop

    :_wait_killed
        set /a "_seconds_elapsed=%PACKER_TIMEOUT%-%_seconds_left%" >nul
        call :info Wait aborted by user after %_seconds_elapsed% seconds [%_seconds_left% of %PACKER_TIMEOUT% remaining]
        goto _wait_last

    :_wait_timed_out
        set /a "_seconds_elapsed=%PACKER_TIMEOUT%-%_seconds_left%" >nul
        call :warning Wait timed out after %_seconds_elapsed% seconds
        goto _wait_last

    :_wait_done
        set /a "_seconds_elapsed=%PACKER_TIMEOUT%-%_seconds_left%" >nul
        call :info Task completed successfully after %_seconds_elapsed% seconds [%_seconds_left% of %PACKER_TIMEOUT% remaining]
        goto _wait_last
