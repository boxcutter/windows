...
for /f "usebackq tokens=2,3,4 delims=. " %%G in ('%OSVersion%') do (
    set _major=%%G
    set _minor=%%H
    set _build=%%I
)
...
if /i "%OSArchitecture%" == "32-bit" (
    set KB2842230_MSU=Windows6.1-KB2842230-x86.msu
) else (
    set KB2842230_MSU=Windows6.1-KB2842230-x64.msu
)

call :locate "%KB2842230_MSU%"
if exist "%located_file%" (
    set _msu_path=%located_file%
    goto found_msu
)

call :download "%KB2842230_URL%"
if %ERRORLEVEL% neq 0 (
    goto exit
)
set _exe_path=%downloaded_file%

set _temp_dir=%PACKER_TMP%\kb2842230

call :run mkdir "%_temp_dir%"

call :unzip "%_exe_path%" "%_temp_dir%"
if %ERRORLEVEL% neq 0 (
    goto exit
)

set _msu_path=%_temp_dir%\%KB2842230_MSU%

if not exist "%_msu_path%" (
    call :error File not found: "%_msu_path%"
    goto exit1
)
...
if "%PACKER_DEBUG%" geq "5" (
    set WUSA_OPTIONS=/log:%TEMP%\%KB2842230_MSU%.log
)

call :run start /wait wusa "%_msu_path%" /quiet /norestart %WUSA_OPTIONS%
