...
if defined PowerShellVersion (
    if %PowerShellVersion% gtr %MIN_POWERSHELL_VERSION% (
        goto exit0
    )
)

call :download "%POWERSHELL_URL%"
if %ERRORLEVEL% neq 0 (
    goto exit
)
set _msu_path=%downloaded_file%
...
if "%PACKER_DEBUG%" geq "5" (
    for %%i in ("%_msu_path%") do (
         set POWERSHELL_MSU=%%~nxi
    )

    set WUSA_OPTIONS=/log:%TEMP%\%POWERSHELL_MSU%.log
)
