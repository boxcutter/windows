...
set _url=%~1
set _dest=%~2
if not defined _dest (
    for %%i in ("%_url%") do (
        set _dest=%CD%\%%~nxi
    )
)

call run: powershell -Command "(New-Object System.Net.WebClient).DownloadFile('%_url%', '%_dest%')" <NUL

if %ERRORLEVEL% equ 0 (
    @call :debug %~nx0 exiting with %ERRORLEVEL%
    @exit /b 0
)

call :warning %~nx0 powershell returned error %ERRORLEVEL%, trying cscript...

call :run cscript //nologo "%~f0?.wsf" "%_url%" "%_dest%"
...
' see https://gist.github.com/BitNetwork/b6f7ef23fac1044e5dfb1187fd1936af
...
if (WScript.Arguments.length < 1) {
  shell.Popup("Usage: _wget <url> (file)");
  WScript.Quit(1);
}
