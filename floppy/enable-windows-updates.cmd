...
:: source: https://github.com/joefitzgerald/packer-windows/blob/master/scripts/microsoft-updates.bat

sc query wuauserv | findstr /r /c:"STATE.*RUNNING" >nul 2>nul

if %ERRORLEVEL% equ 0 (
    call :run sc stop wuauserv
)

call :run reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\WindowsUpdate\Auto Update" /v EnableFeaturedSoftware /t REG_DWORD /d 1 /f

call :run reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\WindowsUpdate\Auto Update" /v IncludeRecommendedUpdates /t REG_DWORD /d 1 /f

call :run cscript //nologo "%~f0?.wsf" %*

sc qc wuauserv | findstr /r /c:"STATE.*STOPPED" >nul 2>nul

if %ERRORLEVEL% neq 0 (
    goto exit0
)

sc qc wuauserv | findstr /r /c:"START_TYPE.*DISABLED" >nul 2>nul

if %ERRORLEVEL% neq 0 (
    call :run sc start wuauserv
)
