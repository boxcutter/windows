...
:: @todo download tee.exe

set _tee_cmd=
set _tee_exe=

where /q $path:_tee.cmd
if %ERRORLEVEL% equ 0 (
    call :debug Found _tee.cmd in PATH:
    :: @TODO REMOVE DEBUGGING
    for %%i in (_tee.cmd) do (
        call :debug %%~$PATH:i
    )
    :: @TODO REMOVE DEBUGGING
    goto exit0
...
set _tee_cmd=%SystemRoot%\_tee.cmd
...
if exist "%located_file%" (
    set _tee_exe=%SystemRoot%\tee.exe
    copy /y "%located_file%" "%_tee_exe%"
)
...
echo.>"%_tee_cmd%"
echo>>"%_tee_cmd%" ^<^^!-- :
echo>>"%_tee_cmd%" @setlocal EnableDelayedExpansion EnableExtensions
echo>>"%_tee_cmd%" @if "%PACKER_DEBUG%" geq "5" (@echo on) else (@echo off)
echo>>"%_tee_cmd%" cscript //nologo "%%~f0?.wsf" %%*
echo>>"%_tee_cmd%" exit /b %%ERRORLEVEL%%
echo>>"%_tee_cmd%" ---^>
echo>>"%_tee_cmd%" ^<job^>^<script language="JScript"^>
echo>>"%_tee_cmd%"    if (WScript.Arguments.Count()==0) {
echo>>"%_tee_cmd%"        WScript.StdOut.Write("Usage: _tee filename [append_flag]");
echo>>"%_tee_cmd%"        WScript.Quit(1);
echo>>"%_tee_cmd%"    }
echo>>"%_tee_cmd%"    var fso = new ActiveXObject("Scripting.FileSystemObject");
echo>>"%_tee_cmd%"    var mode=2;
echo>>"%_tee_cmd%"    if (WScript.Arguments.Count()==2) {mode=8;}
echo>>"%_tee_cmd%"    var out = fso.OpenTextFile(WScript.Arguments(0),mode,true);
echo>>"%_tee_cmd%"    var chr;
echo>>"%_tee_cmd%"    while( ^^!WScript.StdIn.AtEndOfStream ) {
echo>>"%_tee_cmd%"        chr=WScript.StdIn.Read(1);
echo>>"%_tee_cmd%"        WScript.StdOut.Write(chr);
echo>>"%_tee_cmd%"        out.Write(chr);
echo>>"%_tee_cmd%"    }
echo>>"%_tee_cmd%"    WScript.Quit(0);
echo>>"%_tee_cmd%" ^</script^>^</job^>

goto after_create

:found_tee
    echo @"%_tee_exe%" %%* >"%_tee_cmd%"
    goto after_create

:after_create
    if not exist "%_tee_cmd%" (
        call :error Cannot create file "%_tee_cmd%"
        goto exit1
    )
    goto exit0
