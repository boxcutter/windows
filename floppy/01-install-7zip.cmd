...
where /q $path:7z.exe
if %ERRORLEVEL% equ 0 (
    call :debug Found 7z.exe in PATH:
...
call :locate "7z.exe"
set _7z_exe=%located_file%

call :locate "7z.dll"
set _7z_dll=%located_file%
...
call :download "%SEVENZIP_URL%"
if %ERRORLEVEL% neq 0 (
    goto exit
)
set _7z_path=%downloaded_file%
...
if not exist "%_7z_exe%" (
    call :error Failed to find 7z.exe in "%_7z_path%"
    goto exit1
)

if not exist "%_7z_dll%" (
    call :error Failed to find 7z.dll in "%_7z_path%"
    goto exit1
)

:copy_sevenzip

call :run copy /y "%_7z_exe%" "%SystemRoot%\" || goto exit
call :run copy /y "%_7z_dll%" "%SystemRoot%\" || goto exit
