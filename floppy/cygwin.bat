...
if not defined CYGWIN_PACKAGES   (
    set CYGWIN_PACKAGES=openssh
)
if not defined CYGWIN_TRIES      (
    set CYGWIN_TRIES=3
)
if not defined CYGWIN_URL        (
...
call :download "%CYGWIN_URL%"
if %ERRORLEVEL% neq 0 (
    goto exit
)

for /l %%i in (1,1,%CYGWIN_TRIES%) do (
    echo ==^> Installing Cygwin [attempt %CYGWIN_TRY% of %CYGWIN_TRIES%]

    call :run "%downloaded_file%" -a %CYGWIN_ARCH% -q -R %CYGWIN_HOME% -P %CYGWIN_PACKAGES% -s %CYGWIN_MIRROR_URL%
...
if not exist a:\cygwin.sh (
    echo ==^> ERROR: File not found: a:\cygwin.sh
    goto exit1
)
...
call :run netsh advfirewall firewall add rule name="SSHD" dir=in action=allow program="%CYGWIN_HOME%\usr\sbin\sshd.exe" enable=yes
