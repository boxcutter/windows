...
if not exist "%~dp0\_download.cmd" (
    call :error %0: File not found: "%~dp0\_download.cmd"
    goto exit1
)

call :run copy /y "%~dp0\_download.cmd" "%SystemRoot%"
if %ERRORLEVEL% neq 0 (
    goto exit
)

for %%i in ("%WGET_URL%") do (
    set _wget_exe=%%~nxi
)

where /q $path:%_wget_exe%
if %ERRORLEVEL% equ 0 (
    call :debug Found "%_wget_exe%" in PATH:
    :: @TODO REMOVE DEBUGGING
    for %%i in (%_wget_exe%) do (
        call :debug %%~$PATH:i
    )
    :: @TODO REMOVE DEBUGGING
    goto exit0
...
if exist "%located_file%" (
    set _wget_path=%located_file%
    goto found
)

set _wget_path=%PACKER_TMP%\%_wget_exe%

call :run call _download.cmd "%WGET_URL%" "%_wget_path%"
if %ERRORLEVEL% neq 0 (
    goto exit
)
...
call :run copy /y "%_wget_path%" "%SystemRoot%\%_wget_exe%"
