...
for /f "tokens=*" %%f in ('wmic computersystem get AutomaticManagedPagefile /value ^| find "="') do (
  @set "%%f"
)

if not '%AutomaticManagedPagefile%'=='FALSE' (
  echo ==^> Configuring pagefile automatic management to False
  call :run wmic computersystem set AutomaticManagedPagefile=False
)

set AutomaticManagedPagefile=

echo ==^> Checking pagefile size

for /f "tokens=*" %%f in ('wmic pagefileset where name^="%SystemDrive%\\pagefile.sys" get InitialSize^,MaximumSize /value ^| find "="') do (
  set "%%f"
)

set _wrongsize=
if not '%InitialSize%'=='%PACKER_PAGEFILE_MB%' (
    set _wrongsize=1
)
if not '%MaximumSize%'=='%PACKER_PAGEFILE_MB%' (
    set _wrongsize=1
)

if defined _wrongsize (
  echo ==^> Configuring pagefile size
  call :run wmic pagefileset where name="%SystemDrive%\\pagefile.sys" set InitialSize=%PACKER_PAGEFILE_MB%,MaximumSize=%PACKER_PAGEFILE_MB%
    set _wrongsize=
)
