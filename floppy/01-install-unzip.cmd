...
if %ERRORLEVEL% equ 0 (
    call :debug Found _unzip.cmd in PATH
    :: @TODO REMOVE DEBUGGING
    for %%i in (_unzip.cmd) do (
        call :debug %%~$PATH:i
    )
    :: @TODO REMOVE DEBUGGING
    goto exit0
...
set _unzip_cmd=%SystemRoot%\_unzip.cmd

for %%i in (unzip.exe) do (
    set _unzip_exe=%%~$PATH:i
)
if exist "%_unzip_exe%" (
    goto found_unzip
)
...
call :locate "_unzip.cmd"
if exist "%located_file%" (
    copy /y "%%located_file%%" "%_unzip_cmd%"
    goto after_create
)

call :locate "unzip.exe"
if exist "%located_file%" (
    set _unzip_exe=%SystemRoot%\unzip.exe
    copy /y "%located_file%" "%_unzip_exe%"
    goto found_unzip
)

call :locate "7z.exe"
if not exist "%located_file%" (
    goto create_unzip_cmd
)
...
copy /y "%_7z_exe%" "%SystemRoot%" || goto exit
copy /y "%_7z_dll%" "%SystemRoot%" || goto exit

set _7z_exe=%SystemRoot%\7z.exe
goto found_7zip

:found_unzip
    echo @"%_unzip_exe%" -o -j -d "%%~2" "%%~1" >"%_unzip_cmd%"
    goto after_create

:found_7zip
    echo @"%_7z_exe%" e -y -o"%%~2" "%%~1" >"%_unzip_cmd%"
    goto after_create
...
echo>>"%_unzip_cmd%" ^<^^!-- :
echo>>"%_unzip_cmd%" @setlocal EnableDelayedExpansion EnableExtensions
echo>>"%_unzip_cmd%" @if "%PACKER_DEBUG%" geq "5" (@echo on) else (@echo off)
echo>>"%_unzip_cmd%" cscript //nologo "%%~f0?.wsf" %%*
echo>>"%_unzip_cmd%" exit /b %%ERRORLEVEL%%
echo>>"%_unzip_cmd%" ---^>
echo>>"%_unzip_cmd%" ^<job^>^<script language="VBScript"^>
echo>>"%_unzip_cmd%"    Set fso = CreateObject("Scripting.FileSystemObject")
echo>>"%_unzip_cmd%"    If Wscript.Arguments.Count ^< 1 Then
echo>>"%_unzip_cmd%"        WScript.StdOut.Write("Usage: _unzip zipfile [destdir]")
echo>>"%_unzip_cmd%"        WScript.Quit(1)
echo>>"%_unzip_cmd%"    End If
echo>>"%_unzip_cmd%"    ZipFile=fso.GetAbsolutePathName(Wscript.Arguments(0))
echo>>"%_unzip_cmd%"    If Wscript.Arguments.Count ^> 1 Then
echo>>"%_unzip_cmd%"        ExtractTo=fso.GetAbsolutePathName(Wscript.Arguments(1))
echo>>"%_unzip_cmd%"    Else
echo>>"%_unzip_cmd%"        ExtractTo=fso.GetAbsolutePathName(".")
echo>>"%_unzip_cmd%"    End If
echo>>"%_unzip_cmd%"    If NOT fso.FolderExists(ExtractTo) Then
echo>>"%_unzip_cmd%"        fso.CreateFolder(ExtractTo)
echo>>"%_unzip_cmd%"    End If
echo>>"%_unzip_cmd%"    set objShell = CreateObject("Shell.Application")
echo>>"%_unzip_cmd%"    set FilesInZip=objShell.NameSpace(ZipFile).items
echo>>"%_unzip_cmd%"    objShell.NameSpace(ExtractTo).CopyHere(FilesInZip)
echo>>"%_unzip_cmd%"    Set fso = Nothing
echo>>"%_unzip_cmd%"    Set objShell = Nothing
echo>>"%_unzip_cmd%"    WScript.Quit(0)
echo>>"%_unzip_cmd%" ^</script^>^</job^>

goto after_create

:after_create
    if not exist "%_unzip_cmd%" (
        call :error Cannot create file "%_unzip_cmd%"
        goto exit1
    )
    goto exit0
