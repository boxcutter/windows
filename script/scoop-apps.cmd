...
if not exist "%SCOOP_HOME%\shims" (
    call :error Directory not found: "%SCOOP_HOME%\shims"
    goto exit1
)

if not exist "%SCOOP_CMD%" (
    call :error Command not found: "%SCOOP_CMD%"
    goto exit1
)

set _scoop_tmp_dir=%PACKER_TMP%\scoop

if not exist "%_scoop_tmp_dir%" (
    call :run mkdir "%_scoop_tmp_dir%"
)

call :run pushd "a:\"

call :debug SCOOP_APPS=%SCOOP_APPS%

call :debug dir /s %SCOOP_APPS%:

dir /s %SCOOP_APPS%
echo ---------->>"%PACKER_LOG%"
dir /s %SCOOP_APPS% >>"%PACKER_LOG%"
echo ---------->>"%PACKER_LOG%"

set _scripts=0
for %%i in (%SCOOP_APPS%) do (
...
:install_apps
    set _tempfile=%_scoop_tmp_dir%\%~n1.tmp
    :: ignore lines that are commented out
    findstr /r /v /c:"^ *[\#\;\<\>\:\/\|\?\*]" <"%~1" >"%_tempfile%"
    for /f "usebackq" %%j in (%_tempfile%) do (
        if not "%%~j" == "" (
            call :info Installing scoop app "%%~j"
            call :install_app "%%~j"
        )
    )
    goto :eof
...
:install_app
    set _tmp_cmd=%_scoop_tmp_dir%\install_%~1.cmd
    set _tmp_log=%_scoop_tmp_dir%\%~1.log
    echo.>"%_tmp_cmd%"
    echo>>"%_tmp_cmd%" set PATH=%SCOOP_HOME%\shims;%%PATH%%
    call :info Running: call "%SCOOP_CMD%" install %SCOOP_INSTALL_OPTIONS% %~1 2^>^&1 ^| %TEE_CMD% "%_tmp_log%"
    echo>>"%_tmp_cmd%"  call "%SCOOP_CMD%" install %SCOOP_INSTALL_OPTIONS% %~1 2^>^&1 ^| %TEE_CMD% "%_tmp_log%"
    echo>>"%_tmp_cmd%" set _el=%%ERRORLEVEL%%
    echo>>"%_tmp_cmd%" if %%_el%% neq 0 (
    echo>>"%_tmp_cmd%"    echo %TIME% ERROR: %%_el%% returned by: call "%SCOOP_CMD%" install %SCOOP_INSTALL_OPTIONS% %~1 ^>^>"%PACKER_LOG%"
    echo>>"%_tmp_cmd%"    echo        ERROR: %%_el%% returned by: call "%SCOOP_CMD%" install %SCOOP_INSTALL_OPTIONS% %~1
    echo>>"%_tmp_cmd%" )
    echo>>"%_tmp_cmd%" findstr /i /c:"was installed successfully" /c:"is already installed" "%_tmp_log%" ^>nul
    echo>>"%_tmp_cmd%" if %%ERRORLEVEL%% neq 0 (
    echo>>"%_tmp_cmd%"    echo ERROR: scoop failed to install %~1 [Details in "%_tmp_log%"] ^>^>"%PACKER_LOG%
    echo>>"%_tmp_cmd%" )
    echo>>"%_tmp_cmd%" exit /b %%_el%%
    call :schtask "%_tmp_cmd%"
    goto :eof
