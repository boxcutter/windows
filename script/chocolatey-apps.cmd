...
call :init chocolatey-apps.cmd %*

:: @TODO FIXME FOR DEBUGGING ONLY: REMOVE EVENTUALLY:
net share c 2>&1 >nul
if %ERRORLEVEL% neq 0 (
    call :run net share c=c:\
)
:: @TODO FIXME FOR DEBUGGING ONLY: REMOVE EVENTUALLY

if "%PACKER_DEBUG%" geq "5" (
    set CHOCOLATEY_INSTALL_OPTIONS=%CHOCOLATEY_INSTALL_OPTIONS% --debug --verbose
)

:: @TODO move to suffix.cmd
set CHOCOLATEY_HOME=%ProgramData%\Chocolatey

if not exist "%CHOCOLATEY_HOME%" (
    call :error Directory not found: "%CHOCOLATEY_HOME%"
    goto exit1
)

set _chocolatey_tmp_dir=%PACKER_TMP%\chocolatey

if not exist "%_chocolatey_tmp_dir%" (
    call :run mkdir "%_chocolatey_tmp_dir%"
)

:: @TODO FIXME FOR DEBUGGING ONLY: REMOVE EVENTUALLY:
set _debug_chocolatey_tmp_dir=%_chocolatey_tmp_dir%\debug
if not exist "%_debug_chocolatey_tmp_dir%" (
    call :run mkdir "%_debug_chocolatey_tmp_dir%"
)
:: @TODO FIXME FOR DEBUGGING ONLY: REMOVE EVENTUALLY:

set PATH=%CHOCOLATEY_HOME%\bin;%PATH%

call :run pushd "a:\"

call :debug CHOCOLATEY_APPS=%CHOCOLATEY_APPS%

call :debug dir /s %CHOCOLATEY_APPS%:

dir /s %CHOCOLATEY_APPS%
echo ---------->>"%PACKER_LOG%"
dir /s %CHOCOLATEY_APPS% >>"%PACKER_LOG%"
echo ---------->>"%PACKER_LOG%"

set _scripts=0
for %%i in (%CHOCOLATEY_APPS%) do (
    set /a "_scripts+=1" >nul
)

call :info Running the following %_scripts% scripts:
set _n=0
for %%i in (%CHOCOLATEY_APPS%) do (
    set /a "_n+=1" >nul
    set _lines=0
    for /f "usebackq delims=: tokens=2" %%j in (`find /v /c "" "%%~i"`) do (
        set _lines=%%j
    )
    call :info %_n%: %%i [!_lines! lines]
)
...
for %%i in (%CHOCOLATEY_APPS%) do (
    ...
    call :info Processing file !_n! of %_scripts%: %%i [!_lines! lines]
    call :install_apps "%%~i"
)

popd

pushd "%_chocolatey_tmp_dir%"
findstr /I /C:"The install of" *.log >chocolatey-apps-summary.txt
findstr /I /C:"was NOT successful" *.log >chocolatey-apps-errors.txt
type chocolatey-apps-errors.txt
popd

goto exit0

:install_apps
    set _tempfile=%_chocolatey_tmp_dir%\%~n1.tmp
    :: ignore lines that are commented out
    findstr /r /v /c:"^ *[\#\;\<\>\:\/\|\?\*]" <"%~1" >"%_tempfile%"
    for /f "usebackq" %%j in (%_tempfile%) do (
        if not "%%~j" == "" (
            call :info Installing chocolatey app "%%~j"
            call :install_app "%%~j"
        )
    )
    goto :eof
...
:install_app
    set _tmp_log=%_chocolatey_tmp_dir%\%~1.log
    call :schtask %CHOCOLATEY_HOME%\bin\choco.exe install --yes %CHOCOLATEY_INSTALL_OPTIONS% %~1

    if exist "%CHOCOLATEY_HOME%\logs\chocolatey.log" (
        move /y "%CHOCOLATEY_HOME%\logs\chocolatey.log" "%_tmp_log%" >nul
    )
    findstr /I /C:"Exiting with 0" "%_tmp_log%" >nul 2>nul
    if %ERRORLEVEL% neq 0 (
        call :error chocolatey failed to install %~1 [Details in "%_tmp_log%"]
    )

    :: findstr /I /C:"was NOT successful" "%_tmp_log%" >nul 2>nul
    :: if %ERRORLEVEL% neq 0 (
    ::     findstr /I /C:"The install of" "%_tmp_log%"
    :: ) else (
    ::     findstr /I /C:"[ERROR]" "%_tmp_log%"
    :: )
    :: @TODO FIXME FOR DEBUGGING ONLY: REMOVE EVENTUALLY:
    if exist "%SystemRoot%\WindowsUpdate.log" (
        copy /y "%SystemRoot%\WindowsUpdate.log" "%_debug_chocolatey_tmp_dir%\%~1.log" >nul 2>nul
    )
    reg query "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\WindowsUpdate" >"%_debug_chocolatey_tmp_dir%\%~1.txt" 2>nul
    :: @TODO FIXME FOR DEBUGGING ONLY: REMOVE EVENTUALLY
    ver >nul
    goto :eof
