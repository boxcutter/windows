...
call :init uninstall-openssh.cmd %*

if not defined PACKER_SHUTDOWN_SECONDS (
    set PACKER_SHUTDOWN_SECONDS=0
)
...
set _PACKER_UNINSTALL_SSH_CMD=%PACKER_TMP%\packer_uninstall_ssh.cmd
...
call :run schtasks /create /sc once /tn packer_uninstall_ssh /tr "%_PACKER_UNINSTALL_SSH_CMD% >%_PACKER_UNINSTALL_SSH_CMD%.log 2>&1" /st 00:00 /sd 12/31/2099 /f /rl HIGHEST /RU SYSTEM

echo ==^> Running task packer_uninstall_ssh
call :run schtasks /run /tn packer_uninstall_ssh

call :run schtasks /query /tn packer_uninstall_ssh
