...
call :init chocolatey.cmd %*

:: @TODO FOR DEBUGGING ONLY: REMOVE EVENTUALLY:
...
set _chocolatey_bin=%ProgramData%\Chocolatey\bin

if not exist "%_chocolatey_bin%" (
    call :error Directory not found: "%_chocolatey_bin%"
    goto exit1
)

set /p ".=;%_chocolatey_bin%" >>"%PACKER_TMP%\PATH" <nul
set PATH=%PATH%;%_chocolatey_bin%
reg query "HKLM\SYSTEM\CurrentControlSet\Control\Session Manager\Environment" | findstr /i /c:"%_chocolatey_bin%" >nul 2>nul

if %ERRORLEVEL% neq 0 (
    call :run setx PATH "%PATH%" /m
)
