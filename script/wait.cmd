...
if not defined PACKER_WAIT (
    set PACKER_WAIT=60
)

:: @TODO REMOVE THIS

set _body_tmp=%PACKER_TMP%\body.txt
set _errors_tmp=%PACKER_TMP%\errors.txt

findstr /r /c:"\<[E]RROR\>" "%PACKER_LOG%" | findstr /v "ERROR_" >"%_errors_tmp%"

set _filelist="%_errors_tmp%"

set _filelist=%_filelist% "%PACKER_TMP%\install-packages\install-packages-errors.txt"
set _filelist=%_filelist% "%PACKER_TMP%\install-packages\install-packages-summary.txt"
...
set EMAIL_BODY=%_body_tmp%

set _filelist=%_filelist% "%PACKER_LOG%"
set _filelist=%_filelist% "%TEMP%\_run-scripts.log"

set EMAIL_FILES=

for %%i in (%_filelist%) do (
    if exist "%%~i" (
        if defined EMAIL_FILES (
            set EMAIL_FILES=!EMAIL_FILES!*
        )
        set EMAIL_FILES=!EMAIL_FILES!%%~i
    )
)

set | findstr /r /c:"^EMAIL_"

if exist "a:\_email.cmd" (
    call a:\_email.cmd
)

:: @TODO REMOVE THIS

echo ==^> Waiting %PACKER_WAIT% seconds...
echo ==^> Delete %USERPROFILE%\Desktop\DeleteToContinue.txt to continue or
echo ==^> Press Ctrl-C to abort

call :pause %PACKER_WAIT%
