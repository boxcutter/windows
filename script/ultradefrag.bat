...
call :init ultradefrag.bat %*

set _tmp_dir=%PACKER_TMP%\ultradefrag

call :run mkdir "%_tmp_dir%"

call :download "%ULTRADEFRAG_URL%"
if %ERRORLEVEL% neq 0 (
    goto exit
)

call :unzip "%downloaded_file%" "%_tmp_dir%"

pushd "%_tmp_dir%"

set _udefrag_exe=
for /r %%i in (udefrag.exe) do (
    if exist "%%~i" (
        set _udefrag_exe=%%~i
    )
)
...
if not exist "%_udefrag_exe%" (
    call :error File not found: udefrag.exe in "%_tmp_dir%"
    goto exit1
)

call :run "%_udefrag_exe%" --optimize --repeat %SystemDrive%
