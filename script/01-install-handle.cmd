...
call :init 01-install-handle.cmd %*

for %%i in ("%HANDLE_URL%") do (
    set _handle_exe=%%~nxi
)

where /q $path:%_handle_exe%
if %ERRORLEVEL% equ 0 (
    call :debug Found "%_handle_exe%" in PATH
    goto exit0
)

call :download "%HANDLE_URL%"
if %ERRORLEVEL% neq 0 (
    goto exit
)

call :run copy /y "%downloaded_file%" "%SystemRoot%"

call :run reg add "HKCU\Software\Sysinternals\Handle" /v EulaAccepted /t REG_DWORD /d 1 /f
