...
call :init freeze.cmd %*

:: @TODO move this to separate script

set EMAIL_BODY=%PACKER_TMP%\errors.txt

findstr /C:"ERROR" "%PACKER_LOG%" | findstr /v "ERROR_" >"%EMAIL_BODY%"

set EMAIL_FILES="%PACKER_LOG%","%EMAIL_BODY%"
if not defined PACKER_BUILD_NAME (
    set EMAIL_FILES=%EMAIL_FILES%,"%TEMP%\_run-scripts.log"
)

if exist "%PACKER_TMP%\chocolatey-apps\chocolatey-apps-errors.txt" (
    type "%PACKER_TMP%\install-packages\chocolatey-apps-errors.txt" >>"%EMAIL_BODY%"
    set EMAIL_FILES=%EMAIL_FILES%,"%PACKER_TMP%\chocolatey-apps\chocolatey-apps-errors.txt
)
if exist "%PACKER_TMP%\chocolatey-apps\install-packages-summary.txt" (
    type "%PACKER_TMP%\chocolatey-apps\install-packages-summary.txt" >>"%EMAIL_BODY%"
    set EMAIL_FILES=%EMAIL_FILES%,"%PACKER_TMP%\chocolatey-apps\install-packages-summary.txt"
)

call :locate "_email.cmd"
if exist "%located_file%" (
    call :run call "%located_file%"
)

:: @TODO REMOVE THIS

echo ==^> Waiting forever...
echo ==^> Delete %USERPROFILE%\Desktop\DeleteToContinue.txt to continue or
