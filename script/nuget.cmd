...
for %%i in ("%NUGET_URL%") do (
    set _nuget_exe=%%~nxi
)

where /q $path:%_nuget_exe%
if %ERRORLEVEL% equ 0 (
    call :debug Found %_nuget_exe% in PATH
    goto exit0
)

call :download "%NUGET_URL%"
if %ERRORLEVEL% neq 0 (
    goto exit
)

call :copy_to_path "%downloaded_file%"

goto exit0

:copy_to_path
    set copied_to_path_ok=
    for %%i in ("%path:;=";"%") do (
        call :copy_to_path1 "%~1" "%%~i"
    )
    goto :eof

:copy_to_path1
    if defined copied_to_path_ok (
        goto :eof
    )
    call :run copy /y "%~1" "%~2"
    if %ERRORLEVEL% equ 0 (
        set copied_to_path_ok=1
    )
    goto :eof
