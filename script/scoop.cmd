...
if %POWERSHELL_VERSION% lss 3.0 (
    if not exist "a:\01-install-powershell.cmd" (
        call :warn scoop.cmd requires 01-install-powershell.cmd to install PowerShell 4
        goto exit1
    )
)

call :run powershell -command "set-executionpolicy unrestricted -s cu" <nul

call :run powershell -NoProfile -ExecutionPolicy unrestricted -File "%downloaded_file%" <NUL
...
if defined SCOOP_BUCKETS (
    call :run call %SCOOP_CMD% install git %SCOOP_INSTALL_OPTIONS%
    for %%a in (%SCOOP_BUCKETS%) do (
        for /f "tokens=1,2 delims=#" %%b in ("%%a") do (
            call :run call "%SCOOP_CMD%" bucket add %%b %%c
        )
    )
)
