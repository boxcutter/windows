...
set >"%PACKER_TMP%\set@vagrant.txt"

call :init vagrant.bat %*

set AUTHORIZED_KEYS=%USERPROFILE%\.ssh\authorized_keys

call :run mkdir "%USERPROFILE%\.ssh"

call :download "%VAGRANT_PUB_URL%"
if %ERRORLEVEL% neq 0 (
    goto exit
)

call :run type "%downloaded_file%" >>"%AUTHORIZED_KEYS%"

if "%USERNAME%" == "sshd_server" (
    for %%i in (%USERPROFILE%) do (
        set USERNAME=%%~ni
    )
)

call :run wmic USERACCOUNT WHERE "Name='%USERNAME%'" set PasswordExpires=FALSE
