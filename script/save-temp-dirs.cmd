...
call :init save-temp-dirs.cmd %*

if not defined PACKER_LOG_DIR (
    set PACKER_LOG_DIR=z:\c\packer_logs
)

for %%i in ("%PACKER_LOG_DIR%") do (
    if not exist "%%~di\" (
        echo "%PACKER_BUILDER_TYPE%" | findstr /i "vmware" >nul
        if !ERRORLEVEL! equ 0 (
            call :run net use z: "\\vmware-host\Shared Folders"
            call :run net use y: "\\vmware-host\Shared Folders\C"
        )
    )
)

if defined PACKER_VM_NAME (
    if not defined PACKER_BUILD_NAME (
        set PACKER_BUILD_NAME=%PACKER_VM_NAME%
    )
)

set _dir_name=%PACKER_VM_NAME%

set _log_path=%PACKER_LOG_DIR%
if not exist "%_log_path%" (
    mkdir "%_log_path%"
)
set _log_path=%PACKER_LOG_DIR%\%_dir_name%
if not exist "%_log_path%" (
    mkdir "%_log_path%"
)
set _log_path=%PACKER_LOG_DIR%\%_dir_name%\temp
if not exist "%_log_path%" (
    mkdir "%_log_path%"
)
if not exist "%_log_path%" (
    call :warn Unable to create directory "%_log_path%"
    goto exit0
)

call :run xcopy /c /e /h /i /k /r /y "%TEMP%" "%_log_path%\"

call :run xcopy /c /e /h /i /k /r /y "%SystemRoot%\TEMP" "%_log_path%\windows_temp\"
