...
@echo ==^> Cleaning SxS...
call :run Dism.exe /online /Cleanup-Image /StartComponentCleanup /ResetBase
...
set _dirs=
set _dirs=%_dirs% "%LOCALAPPDATA%\Nuget"
set _dirs=%_dirs% "%TEMP%"
...
set _dirs=
set _dirs=%_dirs% "%SystemRoot%\Logs"
set _dirs=%_dirs% "%SystemRoot%\Temp"
set _dirs=%_dirs% "%SystemRoot%\WinSxS\ManifestCache"
...
call :run rmdir /q /s "%SystemDrive%\$Recycle.bin"

@echo ==^> Cleaning "%USERPROFILE%"

pushd "%USERPROFILE%"

for %%i in (VBoxGuestAdditions.iso windows.iso) do (
    if exist "%%~i" (
        call :run del /f "%%~i"
    )
)
...
:clean_dir
    ...
    if not "%~2" == "" (
        %_debug% call :run takeown /d Y /R /f "%~1"

        if "%PACKER_DEBUG%" lss "4" (
            set _nul=^>nul 2^>nul
        )

        %_debug% call :run icacls "%~1" /GRANT:r administrators:F /T /c /q %_nul%
    )

    for %%j in ("%~1\*.*") do (
        if /i not "%%~nxj" equ "%~nx0" (
            %_debug% del /f /q /s "%%~j"
        )
    )
