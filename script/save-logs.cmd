...
if not defined _dir_name (
    set _dir_name=%COMPUTERNAME%
)
...
set _log_path=%PACKER_LOG_DIR%\%_dir_name%\logs
if not exist "%_log_path%" (
    mkdir "%_log_path%"
)
...
set _logs=
set _logs=%_logs% "%TEMP%\*.log"
set _logs=%_logs% "%TEMP%\*.txt"
set _logs=%_logs% "%PACKER_TMP%\*.log"
set _logs=%_logs% "%PACKER_TMP%\*.txt"

for %%i in (%_logs%) do (
    call :run xcopy /c /e /h /i /k /r /y "%%~i" "%_log_path%\"
)
