...
call :init sdelete.cmd %*

call :download "%SDELETE_URL%"
if %ERRORLEVEL% neq 0 (
    goto exit
)

call :run "%downloaded_file%" -z %SystemDrive%

call :run reg add "HKCU\Software\Sysinternals\SDelete" /v EulaAccepted /t REG_DWORD /d 1 /f
